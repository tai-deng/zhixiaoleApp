...
function appPay(phoneNum,phoneAmountId,id,sitefcode,orderid,xtoken,orderTypeid, orderAccid){
    var v = $('#test').html();
    var p = 'payPlus==>false';
    if(typeof plus != 'undefined'){
        p = 'payPlus==>true'
    }
    $('#test').html(v+'<br/>进入支付！:'+'<br/>'+p)
    startWXPAY();
    function startWXPAY()
    {
        //调用pay()方法 发起支付即可
        ...
        function getChannels() {
            if(window.plus){
                plus.payment.getChannels(function(channels) {
                    ...
                    var td = $('#test').html();
                    $('#test').html(td+'<br/>当前通道:'+'<br/>'+JSON.stringify(channel)+'<br/>支付通道all:'+'<br/>'+JSON.stringify(channels))
        ...
        function pay() {
            ...
            function ajax_success_callback(resObj) {
                if (window.plus){
                    ...
                    var pment = $('#test').html();
                    $('#test').html(pment+'<br/>wechat参数:'+res_str)

                }else{
                    mui.toast('需安卓手机APP才能支付，ajax');
                }
            }
            //获取微信支付参数失败的回调函数
            function ajax_error_callback(e) {
                ...
                var pmenterr = $('#test').html();
                $('#test').html(pmenterr+'<br/>wechat失败:'+JSON.stringify(e))
...
//                        window.location.href = location.origin+'/mobileApp/html-kuanDaiPay/payList.html?timestamp='+timestamp+'&&orderId='+orderId;
                mui.openWindow({
                    url:location.origin+'/mobileApp/html-huaFei/result.html?timestamp='+timestamp+'&&orderId='+orderId,
                    id:'payList.html'+"?time="+new Date().getTime(),
                    extras:{}
                });

                var wesu = $('#test').html();
                $('#test').html(wesu+'<br/>wepaysu:'+JSON.stringify(result))
            ...
            function wxpay_error(error) {
                ...
                var weerr = $('#test').html();
                $('#test').html(weerr+'<br/>wepayerr:'+JSON.stringify(error))
